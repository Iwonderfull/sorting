...
def ProviderCreate(num):
    return [num, companyName[num]]


def AnimalCreate(num):
    maturity = random.choice([True, False])
    age = random.randint(3, 70)
    gender = random.choice(["Female", "Male"])
    return [num, random.choice(nickNames), gen_datetime(), maturity, age, gender, random.choice(species)]


def ZooCreate(num):
    return [num, zooName[num], country[num], city[num]]


def EmployeeCreate(num):
    gender = random.choice(["F", "M"])
    age = random.randint(18, 90)
    salary = random.randint(25000, 250000)
    post = random.choice(
        ["veterinarians", "cleaners", "trainers", "construction repairmen", "administration workers"])
    return [num, random.choice(surname), random.choice(name), random.choice(name), gender, age, gen_datetime(),
            salary, post]
...
providers_file_path = path + 'Provider.csv'
animals_file_path = path + 'Animal.csv'
employees_file_path = path + 'Employee.csv'
zoo_file_path = path + 'Zoo.csv'

if num_row > 29:
    FileCreate(zoo_file_path, ['Id_zoo', "NameZoo", "Country", "City"], ZooCreate, 29)
else:
    FileCreate(zoo_file_path, ['Id_zoo', "NameZoo", "Country", "City"], ZooCreate, num_row)

FileCreate(providers_file_path, [["Id_provider", "Name_provider"]], ProviderCreate, num_row)
FileCreate(animals_file_path, ['Id_animal', "Nickname", "Birthday", "Maturity", "Age", "Gender", "Species"], AnimalCreate, num_row)
FileCreate(employees_file_path, ["Id_employee", "Surname", "Name", "Patronymic", "Gener", "Age", "DateOfEmployment", "Salary", "Post"],EmployeeCreate, num_row)
